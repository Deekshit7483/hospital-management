...
public class PercificationActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> precipitation_list = new ArrayList<>();
	...
	private DatabaseReference precipitation = _firebase.getReference("precipitation");
	private ChildEventListener _precipitation_child_listener;
	private Intent i = new Intent();
	
	@Override
	protected void onCreate(Bundle _savedInstanceState) {
		super.onCreate(_savedInstanceState);
		setContentView(R.layout.percification);
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		_precipitation_child_listener = new ChildEventListener() {
			@Override
			public void onChildAdded(DataSnapshot _param1, String _param2) {
				GenericTypeIndicator<HashMap<String, Object>> _ind = new GenericTypeIndicator<HashMap<String, Object>>() {};
				final String _childKey = _param1.getKey();
				final HashMap<String, Object> _childValue = _param1.getValue(_ind);
				precipitation.addListenerForSingleValueEvent(new ValueEventListener() {
					@Override
					public void onDataChange(DataSnapshot _dataSnapshot) {
						precipitation_list = new ArrayList<>();
						try {
							GenericTypeIndicator<HashMap<String, Object>> _ind = new GenericTypeIndicator<HashMap<String, Object>>() {};
							for (DataSnapshot _data : _dataSnapshot.getChildren()) {
								HashMap<String, Object> _map = _data.getValue(_ind);
								precipitation_list.add(_map);
							}
						}
						catch (Exception _e) {
							_e.printStackTrace();
						}
						listview1.setAdapter(new Listview1Adapter(precipitation_list));
		...
		precipitation.addChildEventListener(_precipitation_child_listener);
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			View _view = _v;
			if (_view == null) {
				_view = _inflater.inflate(R.layout.precipitation_histor, null);
			}
			...
			if (precipitation_list.get((int)_position).get("doctor id").toString().equals(FirebaseAuth.getInstance().getCurrentUser().getUid())) {
				if (precipitation_list.get((int)_position).containsKey("doctor name")) {
					textview1.setText(precipitation_list.get((int)_position).get("doctor name").toString());
				}
				if (precipitation_list.get((int)_position).containsKey("medicine")) {
					textview2.setText(precipitation_list.get((int)_position).get("medicine").toString());
				}
				if (precipitation_list.get((int)_position).containsKey("user name")) {
					textview5.setText(precipitation_list.get((int)_position).get("user name").toString());
				}
				if (precipitation_list.get((int)_position).containsKey("date")) {
					textview6.setText(precipitation_list.get((int)_position).get("date").toString());
				}
				if (precipitation_list.get((int)_position).containsKey("image")) {
					Glide.with(getApplicationContext()).load(Uri.parse(precipitation_list.get((int)_position).get("image").toString())).into(imageview1);
				}
			...
			imageview1.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View _view) {
					...
					i.putExtra("image", precipitation_list.get((int)_position).get("image").toString());
					startActivity(i);
				}
			});
