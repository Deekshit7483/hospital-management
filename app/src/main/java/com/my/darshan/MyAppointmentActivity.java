...
public class MyAppointmentActivity extends AppCompatActivity {
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			View _view = _v;
			if (_view == null) {
				_view = _inflater.inflate(R.layout.appointment, null);
			}
			...
			if (appointment_list.get((int)_position).get("user id").toString().equals(FirebaseAuth.getInstance().getCurrentUser().getUid())) {
			...
			button1.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View _view) {
					appointment.child(appointment_list.get((int)_position).get("key").toString()).removeValue();
				}
			});
			...
			if (appointment_list.get((int)_position).get("status").toString().equals("Accepted. ")) {
				textview10.setTextColor(0xFF1B5E20);
			}
