...
public class PaymentHistoryActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> payment_list = new ArrayList<>();
					...
					public void onDataChange(DataSnapshot _dataSnapshot) {
						...
						try {
							...
							for (DataSnapshot _data : _dataSnapshot.getChildren()) {
								...
								payment_list.add(_map);
							}
						}
						catch (Exception _e) {
							_e.printStackTrace();
						}
						listview1.setAdapter(new Listview1Adapter(payment_list));
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			View _view = _v;
			if (_view == null) {
				_view = _inflater.inflate(R.layout.payments, null);
			}
			...
			if (payment_list.get((int)_position).get("doctor id").toString().equals(FirebaseAuth.getInstance().getCurrentUser().getUid())) {
				if (payment_list.get((int)_position).containsKey("doctor name")) {
					textview1.setText(payment_list.get((int)_position).get("doctor name").toString());
				}
				if (payment_list.get((int)_position).containsKey("user name")) {
					textview2.setText(payment_list.get((int)_position).get("user name").toString());
				}
				if (payment_list.get((int)_position).containsKey("fees")) {
					textview3.setText(payment_list.get((int)_position).get("fees").toString());
				}
